...
# //////////////////////////////////
# @api_view(["GET"])
# def get_user_boxes(request, user_id):
#     user_boxes = UserBox.objects.filter(user_id=user_id).all()
#     if not user_boxes:
#         return Response(
#             {"User does not have any boxes"}, status=status.HTTP_404_NOT_FOUND
#         )

#     serializer = serializers.UserBoxSerializer(user_boxes, many=True)
#     return Response(
#         {"UserBoxes": [user_box for user_box in serializer.data]},
#         status=status.HTTP_200_OK,
#     )
